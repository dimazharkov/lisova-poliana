...
class GroupedEffectExperimentUseCase(ExperimentUseCaseContract):
    def __init__(self, stat_evaluator: StatEvaluatorContract, experiment_config: ConfigProviderContract, experiment_hue: str):
    ...
    def _plot_overall(self, data):
        stats = {}
        figures = {}
        g0 = data[data[self.exp_hue] == 0]["median_effect"]
        g1 = data[data[self.exp_hue] == 1]["median_effect"]
        stats["all"] = asdict(self.stat_evaluator.evaluate(g0, g1))

        fig = plt.figure(figsize=(5, 4))
        sns.boxplot(data=data, x=self.exp_hue, y="median_effect", palette="Set2", showfliers=False, width=0.4)
    ...
    def _plot_single_groupings(self, data):
        ...
        for col, title in [
            ...
            sns.boxplot(data=data, x=col, y="median_effect", hue=self.exp_hue, palette="Set2", showfliers=False, width=0.4)
            ...
            for val in data[col].dropna().unique():
                ...
                g0 = subset[subset[self.exp_hue] == 0]["median_effect"]
                g1 = subset[subset[self.exp_hue] == 1]["median_effect"]
    ...
    def _plot_facet_pairs(self, data):
        ...
        for row, col, key, title in pairs:
            ...
            g.map_dataframe(sns.boxplot, x=self.exp_hue, y="median_effect", palette="Set2", showfliers=False, width=0.4)
        ...
        for fixed, val, row, col, key in fixed_triplets:
            ...
            for rv in subset[row].unique():
                for cv in subset[col].unique():
                    ...
                    if part.empty:
                        continue
                    g0 = part[part[self.exp_hue] == 0]["median_effect"]
                    g1 = part[part[self.exp_hue] == 1]["median_effect"]
                    stats[f"{key}:{rv}+{cv}"] = asdict(self.stat_evaluator.evaluate(g0, g1))
        return stats, figures
