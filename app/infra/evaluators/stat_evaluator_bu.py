...
class StatEvaluator:
    ...
    def evaluate(x: pd.Series, y: pd.Series, method: Optional[str] = None) -> StatResult:
        ...
        conclusion = StatEvaluator._get_conclusion_u(stat, safe_p_val, x_len, y_len)
    ...
    @staticmethod
    def _get_conclusion_u(stat_value: float, p_value: float, n_x: int, n_y: int) -> StatConclusion:
        if n_x < 5 or n_y < 5:
            return StatConclusion(
                value=None,
                meaning="insufficient data",
                p_value=round(p_value, 4) if p_value >= StatEvaluator.MIN_P_VALUE else f"< {StatEvaluator.MIN_P_VALUE:.0e}",
                test_value=round(stat_value, 4),
                rank_biserial_r=None,
                interpretation="Too few samples for stable effect estimation."
            )

        # Расчёт rank-biserial correlation (только для U-критерия)
        try:
            r_value = 1 - (2 * stat_value) / (n_x * n_y)
